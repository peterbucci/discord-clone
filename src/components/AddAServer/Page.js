...
Page.Header = ({ children, closeMenu, ...restProps }) => {
  return (
    <Header {...restProps}>
      {children}
      <Close onClick={closeMenu}>
        <CloseIcon width={24} height={24} fill={"primary500"} />
      </Close>
    </Header>
  );
};

Page.Title = ({ children, ...restProps }) => {
  return <Title {...restProps}>{children}</Title>;
};

Page.Subtitle = ({ children, ...restProps }) => {
  return <Subtitle {...restProps}>{children}</Subtitle>;
};

Page.Content = ({ children, ...restProps }) => {
  return <Content {...restProps}>{children}</Content>;
};

Page.Option = ({ children, image, subtitle, iconMargin, ...restProps }) => {
  return (
    <Option {...restProps}>
      <OptionIcon
        margin={iconMargin}
        src={`/assets/add_a_server/${image}.svg`}
      />
      {!subtitle && <OptionText>{children}</OptionText>}
      {subtitle && (
        <OptionTitleWrapper>
          <OptionText color="primary860">{children}</OptionText>
          <OptionSubtitle>{subtitle}</OptionSubtitle>
        </OptionTitleWrapper>
      )}
      <OptionArrow src="/assets/add_a_server/right_arrow.svg" />
    </Option>
  );
};

Page.OptionHeader = ({ children, ...restProps }) => {
  return <OptionHeader {...restProps}>{children}</OptionHeader>;
};

Page.Examples = (props) => {
  return (
    <ExamplesWrapper {...props}>
      <OptionHeader>Invite should look like</OptionHeader>
      <Example>hTKzmak</Example>
      <Example>https://discord.gg/hTKzmak</Example>
      <Example>https://discord.gg/cool-people</Example>
    </ExamplesWrapper>
  );
};

Page.JoinForm = ({ value, setValue, ...restProps }) => {
  return (
    <Form {...restProps}>
      <InputContainer>
        <InputLabel>
...
Page.ServerForm = ({ value, setValue, ...restProps }) => {
  return (
    <Form {...restProps}>
      <InputContainer>
        ...
        <GuidelinesText>
          ...
          <GuidelinesTextLink>Community Guidelines</GuidelinesTextLink>.
        </GuidelinesText>
      </InputContainer>
    </Form>
  );
...
Page.SkipQuestion = ({ nextPage, ...restProps }) => {
  return (
    <SkipQuestion {...restProps}>
      Not sure? You can{" "}
      <SkipQuestionLink onClick={nextPage}>skip this question</SkipQuestionLink>{" "}
      for now.
    </SkipQuestion>
  );
};

Page.Footer = ({ children, ...restProps }) => {
  return <Footer {...restProps}>{children}</Footer>;
};

Page.FooterHeader = ({ children, ...restProps }) => {
  return <FooterHeader {...restProps}>{children}</FooterHeader>;
};

Page.JoinAServerButton = (props) => {
  return <JoinAServerButton {...props}>Join a Server</JoinAServerButton>;
};

Page.BackButton = (props) => {
  return <BackButton {...props}>Back</BackButton>;
};

Page.CreateButton = ({ value, ...restProps }) => {
  return <CreateButton {...restProps}>{value}</CreateButton>;
};
